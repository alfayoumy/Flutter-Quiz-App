...
class QuestionController extends GetxController
  ...
  List<Question> _questions = selectedQuiz
      .map(
        (question) => Question(
            id: question['id'],
  ...
  Future<void> setScore() async {
    await FirebaseFirestore.instance.collection('Scores').doc().set({
      'name': username,
      'email': email,
      'score': ((numOfCorrectAns / questions.length) * 100).round(),
      'datetime': DateTime.now(),
      'category': catName,
    }).catchError((error) => print('Error Saving Score Data!'));
  }
